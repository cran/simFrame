...
demonstrates the use of \pkg{simFrame} \citep{alfons10c} in \proglang{R} 
\citep{R10} for various simulation designs in a practical application. It 
extends the example for design-based simulation in \citet{alfons10d} 
(Example~6.1). Different simulation designs in terms of sampling, contamination 
and missing data are thereby investigated to present the strengths of the 
framework. 
