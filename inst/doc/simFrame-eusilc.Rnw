...
This is a supplementary paper to ``An Object-Oriented Framework for Statistical 
Simulation: The \proglang{R} package \pkg{simFrame}'' \citep{alfons10d} and 
demonstrates the use of \pkg{simFrame} \citep{alfons10c} for various 
simulation designs in a practical application. It extends the example for 
design-based simulation in \citet{alfons10d} (Example~6.1). Different 
simulation designs in terms of sampling, contamination and missing data are 
thereby investigated to present the strengths of the framework. 

Note that the paper is supplementary material and is supposed to be read after 
studying \citet{alfons10d}. It does not give a detailed discussion about the 
motivation for the framework, nor does it describe the design or implementation 
of the package. Instead it is focused on showing its full functionality for 
...
semiparametric approaches are based on fitting a \emph{Pareto} distribution 
\citep[e.g.,][]{kleiber03} to the upper tail of the data. In the first 
approach, the classical \emph{Hill} estimator \citep{hill75} is used to 
estimate the shape parameter of the Pareto distribution, while the second uses 
the robust \emph{partial density component} estimator 
\citep[PDC;][]{vandewalle07}. All these methods are implemented in the 
\proglang{R} package \pkg{laeken} \citep{alfons10b}. For a more detailed 
discussion on Pareto tail modeling in the case of the Gini coefficient and 
a related measure of inequality, the reader is referred to \citet{alfons10f}.
...
#sc <- SampleControl(grouping = "hid", size = 1500, k = 6)
## run simulation
...
#    size = c(75, 250, 250, 125, 200, 225, 125, 150, 100), k = 6)
## run simulation
results <- runSimulation(eusilcP, sc, fun = sim, k = 175)
@
...
the literature \citep[e.g.,][]{little02}: \emph{missing completely at random} 
(MCAR), \emph{missing at random} (MAR) and \emph{missing not at random} (MNAR). 
All three missing data mechanisms are implemented in the framework. 
...
#nc <- NAControl(target = "eqIncome", 
#    NArate = c(0, 0.05), aux = "eqIncome")
...
parallel computing can drastically reduce the computational costs. In 
\pkg{simFrame}, parallel computing is implemented using \pkg{snow} 
\citep{rossini07, tierney08}. Only minimal additional programming 
effort due to the use of \pkg{snow} is required to adapt the code from the 
previous example: to initialize the computer cluster, to ensure that all 
packages and objects are available on each worker process, to use the function 
\code{clusterRunSimulation()} instead of \code{runSimulation()} and to stop the 
computer cluster after the simulations. In addition, random number streams 
\citep[e.g.,][]{lecuyer02, sevcikova09} should be used instead of the built-in 
...
\pkg{simFrame} can easily be done based on package \pkg{snow}.
...
%\citep{leisch02a, leisch02b} with \proglang{R} version~2.11.1 and 
%\pkg{simFrame} version~0.2. 
